...
// JWT SECRET KEY
const JWT_SECRET_KEY = "DUMMYDUMMYDUMMYDUMMYDUMMYDUMMYDUMMYDUMMY";
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  switch (req.method) {
    ...
    case "GET":
      ...
      const accessTokenB = req.headers.cookie;
      if (!accessTokenB) {
        res.statusCode = 400;
        return res.send("access_token이 없습니다.(로그인 되어있지 않습니다.)");
      }
      // console.log(accessTokenB); // accessToken=[토큰값] 의 형태로 찍힘. 이걸 따로 파싱을 해야하나??
      const accessToken = accessTokenB.replace("access_token=", "");
      ...
      return res.status(200).send(user_get);
